...
// iter 3b — test harness:
extern ERL_NIF_TERM nif_ui_tree(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]);
extern ERL_NIF_TERM nif_ui_view_tree(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]);
extern ERL_NIF_TERM nif_screen_info(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]);
extern ERL_NIF_TERM nif_ui_debug(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]);
extern ERL_NIF_TERM nif_ax_action(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]);
extern ERL_NIF_TERM nif_ax_action_at_xy(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]);
extern ERL_NIF_TERM nif_tap(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]);
extern ERL_NIF_TERM nif_tap_xy(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]);
extern ERL_NIF_TERM nif_type_text(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]);
extern ERL_NIF_TERM nif_delete_backward(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]);
extern ERL_NIF_TERM nif_key_press(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]);
extern ERL_NIF_TERM nif_clear_text(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]);
extern ERL_NIF_TERM nif_long_press_xy(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]);
extern ERL_NIF_TERM nif_swipe_xy(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]);

// ── Cached JNI method IDs (definition moved to mob_nif.zig in iter 3b) ──────
// The matching extern struct here is the C view of the same memory the Zig
// side defines and exports. Field order is load-bearing — drift here will
// silently mis-resolve method IDs at runtime. When a Bridge field is added
// or removed, BOTH this declaration AND the BridgeMethods extern struct in
// mob_nif.zig must change together.
struct BridgeMethods {
    jclass cls;
    jmethodID set_root;
...
extern struct BridgeMethods Bridge;

// JNI thread-attach helper (definition moved to mob_nif.zig). The senders
// + feature NIFs still in this file call it like before; the function
// itself is now exported with C ABI from Zig.
...
void mob_send_focus(int handle) {
    send_event(handle, "focus");
}
void mob_send_blur(int handle) {
    send_event(handle, "blur");
}
void mob_send_submit(int handle) {
    send_event(handle, "submit");
}
void mob_send_select(int handle) {
    send_event(handle, "select");
}
...
void mob_send_compose(int handle, const char *text, const char *phase) {
    ...
    ERL_NIF_TERM msg =
        enif_make_tuple3(env, enif_make_atom(env, "compose"), enif_make_copy(env, tag), payload);
    enif_send(NULL, &pid, env, msg);
    enif_free_env(env);
}
...
static int mob_throttle_check_a(int handle, double x, double y, int default_throttle_ms,
...
static ERL_NIF_TERM mob_build_scroll_map(ErlNifEnv *env, double x, double y, double dx, double dy,
                                         double vx, double vy, const char *phase, long long ts_ms,
                                         unsigned long long seq) {
    ERL_NIF_TERM keys[9] = {
    ...
    ERL_NIF_TERM vals[9] = {
        ...
        enif_make_atom(env, phase), enif_make_int64(env, ts_ms), enif_make_uint64(env, seq),
    };
    ERL_NIF_TERM map;
...
void mob_send_scroll(int handle, double x, double y, double dx, double dy, double vx, double vy,
    ...
    if (!phase_boundary && !mob_throttle_check_a(handle, x, y, 33, 1.0))
        return;
    ...
    ERL_NIF_TERM msg =
        enif_make_tuple3(env, enif_make_atom(env, "scroll"), enif_make_copy(env, tag), payload);
    enif_send(NULL, &pid, env, msg);
    enif_free_env(env);
}
...
void mob_send_drag(int handle, double x, double y, double dx, double dy, const char *phase) {
    int phase_boundary = (strcmp(phase, "began") == 0) || (strcmp(phase, "ended") == 0);
    if (!phase_boundary && !mob_throttle_check_a(handle, x, y, 16, 1.0))
        return;
    ...
    ErlNifEnv *env = enif_alloc_env();
    ERL_NIF_TERM keys[7] = {
        enif_make_atom(env, "x"),   enif_make_atom(env, "y"),     enif_make_atom(env, "dx"),
        enif_make_atom(env, "dy"),  enif_make_atom(env, "phase"), enif_make_atom(env, "ts"),
        enif_make_atom(env, "seq"),
    };
    ERL_NIF_TERM vals[7] = {
        enif_make_double(env, x),   enif_make_double(env, y),   enif_make_double(env, dx),
        enif_make_double(env, dy),  enif_make_atom(env, phase), enif_make_int64(env, ts_ms),
        enif_make_uint64(env, seq),
    };
    ERL_NIF_TERM payload;
    enif_make_map_from_arrays(env, keys, vals, 7, &payload);
    ERL_NIF_TERM msg =
        enif_make_tuple3(env, enif_make_atom(env, "drag"), enif_make_copy(env, tag), payload);
    enif_send(NULL, &pid, env, msg);
    enif_free_env(env);
}
...
void mob_send_pinch(int handle, double scale, double velocity, const char *phase) {
    int phase_boundary = (strcmp(phase, "began") == 0) || (strcmp(phase, "ended") == 0);
    if (!phase_boundary && !mob_throttle_check_a(handle, scale, 0, 16, 0.01))
        return;
    ...
    ErlNifEnv *env = enif_alloc_env();
    ERL_NIF_TERM keys[5] = {
        enif_make_atom(env, "scale"), enif_make_atom(env, "velocity"), enif_make_atom(env, "phase"),
        enif_make_atom(env, "ts"),    enif_make_atom(env, "seq"),
    };
    ERL_NIF_TERM vals[5] = {
        enif_make_double(env, scale), enif_make_double(env, velocity), enif_make_atom(env, phase),
        enif_make_int64(env, ts_ms),  enif_make_uint64(env, seq),
    };
    ERL_NIF_TERM payload;
    enif_make_map_from_arrays(env, keys, vals, 5, &payload);
    ERL_NIF_TERM msg =
        enif_make_tuple3(env, enif_make_atom(env, "pinch"), enif_make_copy(env, tag), payload);
    enif_send(NULL, &pid, env, msg);
    enif_free_env(env);
}
...
void mob_send_rotate(int handle, double degrees, double velocity, const char *phase) {
    int phase_boundary = (strcmp(phase, "began") == 0) || (strcmp(phase, "ended") == 0);
    if (!phase_boundary && !mob_throttle_check_a(handle, degrees, 0, 16, 1.0))
        return;
    ...
    ERL_NIF_TERM vals[5] = {
        enif_make_double(env, degrees), enif_make_double(env, velocity), enif_make_atom(env, phase),
        enif_make_int64(env, ts_ms),    enif_make_uint64(env, seq),
    };
    ERL_NIF_TERM payload;
    enif_make_map_from_arrays(env, keys, vals, 5, &payload);
    ERL_NIF_TERM msg =
        enif_make_tuple3(env, enif_make_atom(env, "rotate"), enif_make_copy(env, tag), payload);
    enif_send(NULL, &pid, env, msg);
    enif_free_env(env);
}

void mob_send_pointer_move(int handle, double x, double y) {
    if (!mob_throttle_check_a(handle, x, y, 33, 4.0))
        return;
    ...
    ERL_NIF_TERM msg = enif_make_tuple3(env, enif_make_atom(env, "pointer_move"),
                                        enif_make_copy(env, tag), payload);
    enif_send(NULL, &pid, env, msg);
    enif_free_env(env);
}
...
void mob_send_scroll_began(int handle) {
    send_event(handle, "scroll_began");
}
void mob_send_scroll_ended(int handle) {
    send_event(handle, "scroll_ended");
}
void mob_send_scroll_settled(int handle) {
    send_event(handle, "scroll_settled");
}
void mob_send_top_reached(int handle) {
    send_event(handle, "top_reached");
}
void mob_send_scrolled_past(int handle) {
    send_event(handle, "scrolled_past");
}
...
// ── JNI helpers (get_jenv moved to mob_nif.zig in iter 3b) ──────────────────
...
void _mob_ui_cache_class_impl(JNIEnv *jenv, const char *bridge_class) {
    ...
    if (!cls) {
        LOGE("mob_ui_cache_class: %s not found", bridge_class);
        return;
    }
    ...
    Bridge.set_startup_phase =
        (*jenv)->GetStaticMethodID(jenv, Bridge.cls, "setStartupPhase", "(Ljava/lang/String;)V");
    if (!Bridge.set_startup_phase)
        (*jenv)->ExceptionClear(jenv);
    Bridge.set_startup_error =
        (*jenv)->GetStaticMethodID(jenv, Bridge.cls, "setStartupError", "(Ljava/lang/String;)V");
    if (!Bridge.set_startup_error)
        (*jenv)->ExceptionClear(jenv);
    LOGI("mob_ui_cache_class: %s cached OK", bridge_class);
}

void mob_set_startup_phase(const char *phase) {
    if (!g_jvm || !Bridge.cls || !Bridge.set_startup_phase)
        return;
    int att;
    JNIEnv *env = get_jenv(&att);
...
void mob_set_startup_error(const char *error) {
    if (!g_jvm || !Bridge.cls || !Bridge.set_startup_error)
        return;
    int att;
    JNIEnv *env = get_jenv(&att);
...
void _mob_bridge_init_activity(JNIEnv *env, jobject activity) {
    if (!Bridge.cls) {
        LOGE("_mob_bridge_init_activity: Bridge.cls not cached");
        return;
    }
    jmethodID init =
        (*env)->GetStaticMethodID(env, Bridge.cls, "init", "(Landroid/app/Activity;)V");
...
static ERL_NIF_TERM nif_color_scheme(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    if (!Bridge.get_color_scheme)
        return enif_make_atom(env, "light");
    int att;
    JNIEnv *jenv = get_jenv(&att);
    jstring result =
...
static ERL_NIF_TERM nif_safe_area(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    ...
    JNIEnv *jenv = get_jenv(&att);
    jfloatArray arr =
        (jfloatArray)(*jenv)->CallStaticObjectMethod(jenv, Bridge.cls, Bridge.get_safe_area);
    ...
    return enif_make_tuple4(
        env, enif_make_double(env, (double)vals[0]), enif_make_double(env, (double)vals[1]),
        enif_make_double(env, (double)vals[2]), enif_make_double(env, (double)vals[3]));
}
...
static ERL_NIF_TERM nif_clipboard_get(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    ...
    JNIEnv *jenv = get_jenv(&att);
    jstring result =
        (jstring)(*jenv)->CallStaticObjectMethod(jenv, Bridge.cls, Bridge.clipboard_get);
...
static ERL_NIF_TERM call_bridge_pid_str(ErlNifEnv *env, jmethodID method, ErlNifPid pid,
                                        const char *arg) {
    int att;
    JNIEnv *jenv = get_jenv(&att);
    jlong jpid;
...
static ERL_NIF_TERM call_bridge_pid_str2(ErlNifEnv *env, jmethodID method, ErlNifPid pid,
    ...
    if (j1)
        (*jenv)->DeleteLocalRef(jenv, j1);
    if (j2)
        (*jenv)->DeleteLocalRef(jenv, j2);
    if (att)
        (*g_jvm)->DetachCurrentThread(g_jvm);
    return enif_make_atom(env, "ok");
}
...
void mob_deliver_motion(jlong jpid, double ax, double ay, double az, double gx, double gy,
                        double gz, long long ts) {
    ErlNifPid pid = pid_from_long(jpid);
    ErlNifEnv *e = enif_alloc_env();
...
static void deliver_webview_binary(jlong jpid, const char *tag, const char *utf8) {
    ...
    enif_alloc_binary(len, &bin);
    memcpy(bin.data, utf8, len);
    ERL_NIF_TERM msg = enif_make_tuple3(e, enif_make_atom(e, "webview"), enif_make_atom(e, tag),
...
void mob_deliver_file_result(
    jlong jpid, const char *event, // "camera","photos","files","audio","scan"
    ...
    if (!json_items || strcmp(json_items, "cancelled") == 0) {
        ...
        enif_alloc_binary(el, &eb);
        memcpy(eb.data, event, el);
        ErlNifBinary sb;
        size_t sl = strlen(sub);
        enif_alloc_binary(sl, &sb);
        memcpy(sb.data, sub, sl);
        msg = enif_make_tuple4(e, enif_make_atom(e, "mob_file_result"), enif_make_binary(e, &eb),
                               enif_make_binary(e, &sb), enif_make_binary(e, &jb));
    }
    enif_send(NULL, &pid, e, msg);
    enif_free_env(e);
}
...
void mob_deliver_push_token(jlong jpid, const char *token) {
    ...
    enif_alloc_binary(tl, &tb);
    memcpy(tb.data, token, tl);
    ERL_NIF_TERM msg = enif_make_tuple3(e, enif_make_atom(e, "push_token"),
                                        enif_make_atom(e, "android"), enif_make_binary(e, &tb));
    enif_send(NULL, &pid, e, msg);
    enif_free_env(e);
}
...
void mob_deliver_notification(jlong jpid, const char *json) {
    ...
    enif_alloc_binary(jl, &jb);
    memcpy(jb.data, json, jl);
    ERL_NIF_TERM msg =
        enif_make_tuple2(e, enif_make_atom(e, "mob_launch_notification"), enif_make_binary(e, &jb));
    enif_send(NULL, &pid, e, msg);
    enif_free_env(e);
}
...
static ERL_NIF_TERM nif_request_permission(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    char cap[32];
    enif_get_atom(env, argv[0], cap, sizeof(cap), ERL_NIF_LATIN1);
    ErlNifPid pid;
    enif_self(env, &pid);
...
static ERL_NIF_TERM nif_biometric_authenticate(ErlNifEnv *env, int argc,
    ...
    if (bin.size < sizeof(reason)) {
        memcpy(reason, bin.data, bin.size);
        reason[bin.size] = 0;
    }
    ErlNifPid pid;
    enif_self(env, &pid);
...
static ERL_NIF_TERM nif_camera_capture_photo(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    char qual[16] = "high";
    enif_get_atom(env, argv[0], qual, sizeof(qual), ERL_NIF_LATIN1);
    ErlNifPid pid;
    enif_self(env, &pid);
...
static ERL_NIF_TERM nif_camera_capture_video(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    int max_dur = 60;
    enif_get_int(env, argv[0], &max_dur);
    ErlNifPid pid;
    enif_self(env, &pid);
    char dur_str[16];
    snprintf(dur_str, sizeof(dur_str), "%d", max_dur);
...
static ERL_NIF_TERM nif_audio_play(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    ...
    memcpy(opts, opts_bin.data, opts_bin.size);
    opts[opts_bin.size] = 0;
    ErlNifPid pid;
    enif_self(env, &pid);
...
static ERL_NIF_TERM nif_audio_set_volume(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    ...
    char vol_str[32];
    snprintf(vol_str, sizeof(vol_str), "%.6f", vol);
    int att;
    JNIEnv *jenv = get_jenv(&att);
...
static ERL_NIF_TERM nif_motion_start(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    ...
    snprintf(interval_str, sizeof(interval_str), "%d", interval_ms);
    ErlNifPid pid;
    enif_self(env, &pid);
...
static ERL_NIF_TERM nif_notify_cancel(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    ...
    if (!enif_inspect_binary(env, argv[0], &bin) &&
        ...
        return enif_make_badarg(env);
    char nid[256] = "";
    if (bin.size < sizeof(nid)) {
        memcpy(nid, bin.data, bin.size);
        nid[bin.size] = 0;
    }
    int att;
    JNIEnv *jenv = get_jenv(&att);
...
// ── Test harness NIFs moved to mob_nif.zig (Phase 6b iter 3b) ────────────────
// nif_ui_tree, nif_ui_view_tree, nif_screen_info, nif_ui_debug,
// nif_ax_action{,_at_xy}, nif_tap, nif_tap_xy, nif_type_text,
// nif_delete_backward, nif_key_press, nif_clear_text, nif_long_press_xy,
// nif_swipe_xy + the jstring_to_bin / cstr_to_bin helpers used only by
// them now live in mob_nif.zig. The nif_funcs[] table below resolves them
// via the extern declarations near the top of this file.
...
static ERL_NIF_TERM nif_storage_dir(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    ...
    jstring result =
        (jstring)(*jenv)->CallStaticObjectMethod(jenv, Bridge.cls, Bridge.storage_dir, jloc);
...
static ERL_NIF_TERM nif_storage_save_to_media_store(ErlNifEnv *env, int argc,
    ...
    enif_self(env, &pid);
    ERL_NIF_TERM result =
        call_bridge_pid_str2(env, Bridge.storage_save_to_media_store, pid, path, type);
    free(path);
    return result;
}
...
static ERL_NIF_TERM nif_storage_external_files_dir(ErlNifEnv *env, int argc,
    ...
    jstring result = (jstring)(*jenv)->CallStaticObjectMethod(
        jenv, Bridge.cls, Bridge.storage_external_files_dir, jtype);
    (*jenv)->DeleteLocalRef(jenv, jtype);
    ERL_NIF_TERM ret;
    if (result) {
        const char *utf8 = (*jenv)->GetStringUTFChars(jenv, result, NULL);
...
void mob_deliver_alert_action(const char *action) {
    ...
    ERL_NIF_TERM msg = enif_make_tuple2(e, enif_make_atom(e, "alert"), enif_make_atom(e, action));
    enif_send(NULL, &pid, e, msg);
    enif_free_env(e);
}
...
static ERL_NIF_TERM nif_webview_can_go_back(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    ...
    JNIEnv *jenv = get_jenv(&att);
    jboolean result =
        (*jenv)->CallStaticBooleanMethod(jenv, Bridge.cls, Bridge.webview_can_go_back);
    if (att)
        (*g_jvm)->DetachCurrentThread(g_jvm);
...
static void mob_device_send_atom_payload_android(const char *tag, const char *atom_name,
    ...
    if (!g_device_dispatcher_set)
        return;
    ErlNifEnv *e = enif_alloc_env();
    ERL_NIF_TERM msg = enif_make_tuple3(e, enif_make_atom(e, tag), enif_make_atom(e, atom_name),
...
static int nif_load(ErlNifEnv *env, void **priv, ERL_NIF_TERM info) {
    ...
    if (!Bridge.cls) {
        LOGE("Bridge.cls not cached — was mob_ui_cache_class called?");
        return -1;
    }
    ...
    int att;
    JNIEnv *jenv = get_jenv(&att);
    Bridge.set_root = (*jenv)->GetStaticMethodID(jenv, Bridge.cls, "setRootJson",
                                                 "(Ljava/lang/String;Ljava/lang/String;)V");
    if (!Bridge.set_root) {
        LOGE("nif_load: setRootJson(String,String) not found on MobBridge");
        return -1;
    }

    Bridge.move_to_back = (*jenv)->GetStaticMethodID(jenv, Bridge.cls, "moveToBack", "()V");
    if (!Bridge.move_to_back) {
        LOGE("nif_load: moveToBack() not found on MobBridge");
        return -1;
    }

    Bridge.get_safe_area = (*jenv)->GetStaticMethodID(jenv, Bridge.cls, "getSafeArea", "()[F");
    if (!Bridge.get_safe_area) {
        LOGE("nif_load: getSafeArea() not found on MobBridge");
        return -1;
    }
    ...
    if (!Bridge.haptic) {
        LOGE("nif_load: haptic(String) not found on MobBridge");
        return -1;
    }

    Bridge.clipboard_put =
        (*jenv)->GetStaticMethodID(jenv, Bridge.cls, "clipboardPut", "(Ljava/lang/String;)V");
    if (!Bridge.clipboard_put) {
        LOGE("nif_load: clipboardPut(String) not found on MobBridge");
        return -1;
    }

    Bridge.clipboard_get =
        (*jenv)->GetStaticMethodID(jenv, Bridge.cls, "clipboardGet", "()Ljava/lang/String;");
    if (!Bridge.clipboard_get) {
        LOGE("nif_load: clipboardGet() not found on MobBridge");
        return -1;
    }

    Bridge.share_text =
        (*jenv)->GetStaticMethodID(jenv, Bridge.cls, "shareText", "(Ljava/lang/String;)V");
    if (!Bridge.share_text) {
        LOGE("nif_load: shareText(String) not found on MobBridge");
        return -1;
    }

    Bridge.open_url =
        (*jenv)->GetStaticMethodID(jenv, Bridge.cls, "openUrl", "(Ljava/lang/String;)V");
    if (!Bridge.open_url) {
        LOGE("nif_load: openUrl(String) not found on MobBridge");
        return -1;
    }
...
#define CACHE_OPT(field, name, sig)                                                                \
    ...
    if (!Bridge.field) {                                                                           \
        (*jenv)->ExceptionClear(jenv);                                                             \
        LOGI("nif_load: %s not found (optional)", name);                                           \
    }
