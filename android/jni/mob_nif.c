...
// ── Senders + handle registries + 6 NIFs moved to mob_nif.zig (iter 3c) ─────
// The full mob_send_* family (tap, change_str/bool/float, focus/blur/submit/
// select/compose, gesture senders, throttled scroll/drag/pinch/rotate/
// pointer_move, scroll-began/ended/settled, swipe_with_direction, back,
// component_event) and the TapHandle / ComponentHandle registries with their
// mutexes are now in Zig. Six NIFs that touched those statics moved with
// them: set_root, register_tap, clear_taps, set_transition, register_component,
// deregister_component.
//
// nif_load (below) calls mob_nif_init_state() — also exported from
// mob_nif.zig — to create the mutexes during BEAM init.
extern int mob_nif_init_state(void);

// iter 3c — registry NIFs:
extern ERL_NIF_TERM nif_set_root(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]);
extern ERL_NIF_TERM nif_register_tap(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]);
extern ERL_NIF_TERM nif_clear_taps(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]);
extern ERL_NIF_TERM nif_set_transition(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]);
extern ERL_NIF_TERM nif_register_component(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]);
extern ERL_NIF_TERM nif_deregister_component(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]);
...
// nif_set_root / nif_register_tap / nif_clear_taps / nif_set_transition
// moved to mob_nif.zig (iter 3c).
...
// nif_register_component / nif_deregister_component moved to mob_nif.zig
// (iter 3c) alongside the ComponentHandle registry they touch.
...
static int nif_load(ErlNifEnv *env, void **priv, ERL_NIF_TERM info) {
    ...
    // tap_mutex + component_mutex are defined in mob_nif.zig (iter 3c).
    // mob_nif_init_state creates both. Returns 0 on success, -1 on failure
    // (enif_mutex_create returned NULL) — matches our nif_load return code.
    if (mob_nif_init_state() != 0) {
        LOGE("nif_load: mob_nif_init_state failed (mutex create)");
        return -1;
    }
