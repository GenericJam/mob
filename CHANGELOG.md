...
- **`guides/agentic_coding.md` covers the 0.8 agent surface** (MOB-212): what
  ...
  (`Mob.PostMortem.sweep/0`), the cross-platform differential
  (`Mob.Differential`, `MobDev.Differential.run/3`), render timing
  (`Mob.RenderStats`), and the mob_dev verification tasks (`mix mob.attest`,
  `mix mob.deploy --json`, `mix mob.mutate`, `mix mob.flake`). The standard
  loop now attests after a push and reads the receipt when state did not
  change. The `Mob.Test` platform matrix no longer says `view_tree/1` is
  `not_loaded` on Android: it works in apps generated by mob_new 0.4.33 or
  newer (MOB-157).
...
- **Physical iOS: every launch crashed before the BEAM printed a line**
  ...
  `mob_notify_window_connected`, `mob_notify_color_scheme` and
  `mob_handle_back` (the boot-splash edge swipe) now return early until
  `nif_load` has run (`mob_runtime_up/0`), and again after `erl_start`
  returns. Affects apps generated by mob_new 0.4.33 and later against mob
  0.8.0+; they pick the fix up on their next native build, no template
  change needed.
...
### Added
- **`max_lines` on `:text`.** A positive integer caps the rendered line count
  and tail-ellipsises the overflow: `.lineLimit(n)` + `.truncationMode(.tail)`
  on iOS, `maxLines` + `TextOverflow.Ellipsis` on Android. Unset means what it
  meant before — wrap without limit — so no existing tree renders differently.
  Only native layout knows the measured width, so this could not be done from
  Elixir; trimming the string by grapheme count was the only workaround, and
  it was a guess. `nil` is dropped so a conditional prop stays safe; any other
  value outside the shared platform range of 1 through 2,147,483,647 raises at
  render time. See `decisions/2026-09-07-max-lines-is-a-native-prop.md`.

  Android reads the prop in the app's own generated `MobBridge.kt`, so it
  needs an app generated by the matching `mob_new` release (or a regenerated
  bridge); an older bridge ignores the key and wraps as before.
- **Measured `Wrap` layout on iOS and Android** (MOB-175). `<Wrap>` greedily
  packs children using their rendered native widths and reflows them when
  content or authored text size changes. `spacing` and `run_spacing` accept
  theme spacing tokens, intrinsically oversized children are proposed the
  container width, and `fill_width: true` children occupy a full run. Explicit
  `fill_width: false` now makes a `Box` hug its content while an omitted value
  preserves the historical full-width Box default. The Android renderer ships
  in the matching `mob_new` generated template.
