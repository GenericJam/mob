...
| Package | Gives you | Notes |
|---|---|---|
| [Mishka Chelekom](https://mishka.tools/chelekom) | 60+ pre-styled components — sliders, dialogs, avatars, menus, pickers, and more — ported to real SwiftUI and Compose | Reads Mob's theme tokens (see [Theming](theming.md)), so switching `Mob.Theme.set/1` re-skins every ported component at once, identically on both platforms. Not yet its own Hex package — the port lives in the [mishka_chelekom monorepo](https://github.com/mishka-group/mishka_chelekom/tree/master/development/mob) alongside the web version. Register only the components a screen actually uses; the demo app's showcase eagerly registers and renders all 60 at boot for its own catalog purposes, which is fine there but adds measurable startup cost on lower-end Android hardware if copied wholesale. |
