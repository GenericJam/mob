...
reported a restart over a dead process, and every app generated by mob_new
0.4.33 or later against mob 0.8.x was dead on physical iOS.
...
false, and it is cleared again if `erl_start` ever returns. The
window-connected notification is only needed by a screen that
...
- Any new `mob_notify_*` entry point reachable from UIKit must check
  `mob_runtime_up()` first; today that is the window-connected and colour
  scheme notifiers and `mob_handle_back` (the edge-swipe recognizer the root
  controller installs before boot). Senders driven by the BEAM's own render tree
